package tencent.liteav.demo.superplayer.ui.view;
...
public class PointSeekBar extends RelativeLayout {
    ...
    public boolean mIsOnDrag;              // 是否处于拖动状态
...
//    }
    ...
    private boolean handleUpEvent(MotionEvent event) {
        float x = event.getX();
        float y = event.getY();
        if (null != upEventListener) {
            upEventListener.upEvent();
        }
    ...
    public UpEventListener upEventListener;

    public interface UpEventListener {
        void upEvent();
    }

    public void setUpEventListener(UpEventListener upEventListener){
        this.upEventListener = upEventListener;
    }

    public MoveEventListener moveEventListener;
